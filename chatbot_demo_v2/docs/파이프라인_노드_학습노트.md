...
> 실제 구현(2026-07-28 기준)과 1:1로 대조해 정리했습니다.
>
...
### ⑪ `web_search_answer` 🔵 *(기본 비활성)*

| 항목 | 내용 |
|---|---|
| **코드** | [graph/nodes.py:563](../graph/nodes.py#L563) · 기본 프로바이더 `DisabledWebSearchProvider` |

`WEB_SEARCH_ENABLED=false` 가 기본이므로 **현재 시연에서는 도달하지 않는 노드**입니다.
확장 지점(인터페이스)만 열어 둔 상태이고, 활성화 조건은 `WEB_SEARCH_SCOPE ∈
{in_domain_unresolved, any_unresolved}` 입니다. 결과는 `final_formatter` 로 직행합니다.
...
| `rag3x` | **재시도 안 함.** `confidence="low"` 강등 + 사용자 경고문구 | 같은 질문으로 RAG 를 다시 돌리면 결과가 같고 지연만 2배(캐시가 있으면 완전히 동일) |
...
| **코드** | [graph/nodes.py:736](../graph/nodes.py#L736) |
...
1. `timings.total_s` = `now - _turn_started_at`
2. **`messages` 에 `AIMessage` 추가** → 다음 턴의 `contextualize_query` 가 이걸 읽습니다
3. `attach_run_metadata()` — session/route/confidence/elapsed/rag_run_id 를 LangSmith 루트 run 에
...
| `CLARIFY_MIN_SCORE` | **0.35** | `.env` | 되묻기 하한(의미 유사도 스케일) |
...
> 0.386 으로 **아슬아슬하게 탈락**) → **`0.35` 채택**. RAG 질문들의 best 는 0.2 이하에 몰려 있어
> 여유가 남습니다.
...
> **부수 효과**: 캔버스가 1560×1385 → **1820×1400** 으로 넓어졌습니다. 슬라이드에 넣을 때
> 가로 비율이 달라지므로 배치를 한 번 확인하세요.
